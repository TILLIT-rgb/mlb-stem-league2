function renderWCHand(gs, teamIdx, timing, WILD_CARD_DEFS, BATTERS, PITCHERS) {
  ...
  const bt = gs.half;
  const ft = 1 - gs.half;
  let html = `<div class="wc-section"><h4>${fmtWildCardsTitle(gs.teams[teamIdx].name)}</h4><div class="wc-hand" id="wcHand${teamIdx}">`;
  wcs.forEach((wi, i) => {
    ...
    html += `<div class="wc-card ${playable ? 'playable' : ''}" ${playable ? `onclick="emitPlayWildCard(${i},'${timing}')"` : ''}>
      ...
      <div class="wc-name">${wcName(wc)}</div>
      <span class="wc-type ${wc.type}">${wc.type === 'bat' ? t('tag_bat') : t('tag_pitch')}</span>
      <div class="wc-desc">${wcDesc(wc)}</div>
      <div class="tap-label">${t('tap_to_play')}</div></div>`;
  });
  html += `</div></div>`;
  return html;
}
